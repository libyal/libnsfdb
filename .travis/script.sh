...
then
	docker run -t -v "${PWD}:/libnsfdb" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libnsfdb && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libnsfdb;
