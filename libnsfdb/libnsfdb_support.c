...
const char *libnsfdb_get_version(
             void )
{
	return( (const char *) LIBNSFDB_VERSION_STRING );
}
...
int libnsfdb_get_access_flags_read(
     void )
{
	return( (int) LIBNSFDB_ACCESS_FLAG_READ );
}
...
{
	static char *function = "libnsfdb_get_codepage";
...
int libnsfdb_set_codepage(
     ...
     libcerror_error_t **error )
{
	static char *function = "libnsfdb_set_codepage";
...
/* Determines if a file is a NSFDB file
     ...
     libcerror_error_t **error )
{
	...
	static char *function            = "libnsfdb_check_file_signature";
...
{
	libbfio_handle_t *file_io_handle = NULL;
	static char *function            = "libnsfdb_check_file_signature_wide";
	size_t filename_length           = 0;
	int result                       = 0;
...
/* Determines if a file is a NSFDB file using a Basic File IO (bfio) handle
 * Returns 1 if true, 0 if not or -1 on error
 */
     ...
     libcerror_error_t **error )
{
	...
	static char *function      = "libnsfdb_check_file_signature_file_io_handle";
	     ...
	     error ) == -1 )
	{
		libcerror_error_set(
		 ...
		 "%s: unable to seek signature offset: 0.",
		 function );
