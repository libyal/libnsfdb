...
void (*nsfdbtools_signal_signal_handler)( nsfdbtools_signal_t ) = NULL;
             ...
             unsigned long signal )
{
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( nsfdbtools_signal_signal_handler != NULL )
			{
				nsfdbtools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "nsfdbtools_signal_attach";
...
#if defined( _MSC_VER )
	nsfdbtools_signal_initialize_memory_debug();
#endif
     ...
     libcerror_error_t **error )
{
	...
	nsfdbtools_signal_signal_handler = NULL;
