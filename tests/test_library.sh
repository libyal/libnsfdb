...
LIBRARY_TESTS="bucket error io_handle note note_item_class note_item_type note_value notify rrv_bucket rrv_bucket_descriptor rrv_value unique_name_key";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./nsfdb_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libnsfdb" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
